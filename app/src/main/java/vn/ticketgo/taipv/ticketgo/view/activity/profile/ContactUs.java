...
public class ContactUs extends FragmentActivity implements OnMapReadyCallback, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(vn.ticketgo.taipv.ticketgo.R.layout.activity_contact_us);
//        ActionBar actionBar=getSupportActionBar();
//        actionBar.setTitle("ContactUs");
        SupportMapFragment mapFragment = (SupportMapFragment) getSupportFragmentManager()
                .findFragmentById(vn.ticketgo.taipv.ticketgo.R.id.map);
        mapFragment.getMapAsync(this);
        init();
    }

    private void init() {
        hotlineHN = findViewById(vn.ticketgo.taipv.ticketgo.R.id.hotline_hn);
        hotlineHCM = findViewById(vn.ticketgo.taipv.ticketgo.R.id.hotline_hcm);
...
//        mMap.animateCamera(CameraUpdateFactory.newLatLngZoom(sydney, 18.0f));
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case vn.ticketgo.taipv.ticketgo.R.id.hotline_hn:
                Intent intent = new Intent(Intent.ACTION_DIAL);
                intent.setData(Uri.parse("tel:02437880099"));
                startActivity(intent);
                break;
            case vn.ticketgo.taipv.ticketgo.R.id.hotline_hcm:
